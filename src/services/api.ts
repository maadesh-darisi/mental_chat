...
export async function chatWithOpenRouter(userInput: string): Promise<ChatResponse> {
  try {
    ...
    const headers = {
      ...
      "X-Title": "Chat Application" // Recommended by OpenRouter
    };
    ...
    return { 
      response: "Sorry, there was an error connecting to the chat service. Please check your API key configuration.", 
      sentiment: "NEUTRAL" 
    };
  }
}
